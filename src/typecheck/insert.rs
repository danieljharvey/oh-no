...
use super::super::types::{Insert, ScalarType,Columns,Table, TableName,  TypeError};
use std::collections::BTreeMap;

fn get_table<'a>(tables: &'a BTreeMap<TableName,Table>, table_name: &'a TableName)
    -> Result<&'a Table,TypeError> {
    tables.get(table_name).ok_or(TypeError::TableNotFound(table_name.clone()))
...
pub fn typecheck_insert(
    ...
    fn table_doesnt_exist() {
        ...
        assert_eq!(typecheck_insert(&tables,&insert),
    ...
    fn column_is_missing() {
        ...
        assert_eq!(typecheck_insert(&tables,&insert),
            Err(TypeError::MissingColumnInInput {table_name: TableName("Horses".to_string()),
